...
export function Sidebar({
    ...
    const mobilePrimaryHrefs = new Set(['/', '/pos', '/orders']);
    ...
    const visibleGroups = sidebarGroups
        .map((group) => ({
            ...
            items: group.items.filter((item) => {
                ...
                if (hidePrimaryNav && mobilePrimaryHrefs.has(item.href)) return false;
                return true;
            }),
        }))
    ...
    return (
        <div className="flex flex-col h-screen bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-white/5 shadow-2xl z-20 overflow-hidden">
            ...
            {/* Logout Button */}
            <div className="p-4 bg-slate-50 dark:bg-black/20 backdrop-blur-xl border-t border-slate-200 dark:border-white/5">
                <button
                    onClick={() => logout()}
                    className="flex w-full items-center gap-3.5 rounded-2xl px-5 py-4 text-sm font-black text-rose-600 dark:text-rose-400 transition-all duration-300 hover:bg-rose-500/10 hover:text-rose-700 dark:hover:text-rose-300 group shadow-sm hover:shadow-rose-500/5"
                >
